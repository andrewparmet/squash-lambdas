...
class ReservationMaker(
    ...
    suspend fun makeReservation(date: LocalDate, otherPlayer: Player? = null): Result {
        val failures = mutableListOf<ReservationResp.NonSuccess>()

        for (time in options.startTimes) {
            for (court in options.courts) {
    ...
    private suspend fun attemptReservation(
        date: LocalDate,
        court: Court,
        startTime: LocalTime,
        player: Player?
